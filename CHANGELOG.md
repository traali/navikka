...
### Fixed
- **Hunter dummy-data + Friday weather storm (2026-08-23)**: Compact MET no longer mints 232° / 1012 hPa / 82% RH / 14 km vis. Companion HUD wind and MET/AIS HTTP wait for LIVE GPS (Helsinki pin is chart, not the boat). Snap-line GPS jitter is not a new MET cell (2 km hysteresis). Persist last `weatherPos` so reload is not `moved`. UKC uses catalog depth − draft (wave height is not a tide). Voyage recap does not invent 5.0 m/s. Missing FMI watlev is omitted, not 0 cm. Flutter weather controller starts idle and runs staggered startup on first GPS (was stuck `isSyncing` forever). Failed weather sources back off 60 s. Emergency cockpit mode opens the MRCC dialog. Gemma tile says the file is not wired to inference. Voice FAB labeled as chip commands, not STT. Skipper does not copy mean wind into “puuskat”.
- **Own-ship honesty (Helsinki pin is not the boat)**: HUD SOG stays "—" until LIVE GPS (0.0 kn is not a log). Recenter/follow no-ops without a fix. MAYDAY/copy/share omit the Kaivopuisto default DDM. Skipper ctx and "lähin satama" wait for GPS. AIS CPA/collision is GPS-gated. Seed MEGASTAR is never painted. MET missing wind/temp throws; missing fog is vis "—" not invented 14 km "Näkyvyys hyvä." Compact MET does not invent dew 11.1 °C or gusts as wind×1.4. Harbor/waypoint NM wait for GPS. Flutter skipper no longer calls missing wind/waves "erinomainen … 0.0 m/s / aallokko 0.0 m". Boating HUD and bottom telemetry hide until `hasLocation`. Forecast "model disagreement" no longer synthesizes missing MET/OW winds.
- **Gauntlet overnight (honesty + radio)**: Skipper no longer invents `Luottamus 94%/96%/88–98%` or stamps template text as AI inference. Flutter weather sync waits for a real GPS fix (Helsinki pin is not the boat) — including the 5 min lightning/alerts timer. Companion does not persist AIS/GPS (`merge` refuses a stored `pos`); SW tile **and** shell caches are LRU-capped; CriOS null **or 0** `speed` derives SOG from movement; own-ship marker **and heading** wait for LIVE GPS; HUD UKC/COG/accuracy stay "—" until a fix. ENC copy is Traficom merikartta (raster), not S-57. Wave sheet no longer says Tyyni without IMU. Search peeks a harbor instead of teleporting own-ship. Missing MET ocean is "—", not 0.6 m. MAYDAY warns if GPS is not LIVE. Skipper does not call 0.0 m/s "excellent" when weather is missing.
- **Companion cold start was a Helsinki demo movie**: 6.2 kn / MEGASTAR seed / fake GPS. Now 0 kn, empty AIS, **GPS…** until a real fix. Last weather/chart persist for offline; SOG does not. ENC layer = Traficom merikartta (official FI raster, not S-57). Production service worker caches shell + tiles. Skipper ask is on-device (Chrome Nano or rules), not a demo LLM. Flutter PWA `NetworkTileProvider` set `User-Agent` + `Referer` (Chrome forbids those → Traficom CORS 403). Companion night theme used **Esri Ocean** — at Helsinki z13 those tiles *are* the watermark. Flutter: no custom tile headers, OSM underlay, transparent error tiles. Companion `/cockpit/`: Carto dark + Traficom merikartta + OpenSeaMap. Native Drift cache unchanged.
