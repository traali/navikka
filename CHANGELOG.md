...
### Fixed
- **Web chart blank ("Map data not yet available")**: Flutter PWA `NetworkTileProvider` set `User-Agent` + `Referer`. Chrome forbids those on `fetch`; Traficom CORS preflight returns **403**, so every nautical tile failed while buoys/AIS still painted. Web tiles now use the browser's own headers; OSM sits under Traficom and failed tiles are transparent (not the grey error card). Native Drift cache unchanged.
