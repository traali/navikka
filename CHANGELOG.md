...
### Fixed
- **Gauntlet overnight (honesty + radio)**: Skipper no longer invents `Luottamus 94%/96%/88–98%` or stamps template text as AI inference. Flutter weather sync waits for a real GPS fix (Helsinki pin is not the boat). Companion does not persist AIS/GPS; SW tile cache is LRU-capped; CriOS null `speed` derives SOG from movement; own-ship marker waits for LIVE GPS. ENC copy is Traficom merikartta (raster), not S-57. Wave sheet no longer says Tyyni without IMU.
