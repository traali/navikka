...
### Fixed
- **Web chart blank ("Map data not yet available")**: Flutter PWA `NetworkTileProvider` set `User-Agent` + `Referer`. Chrome forbids those on `fetch`; Traficom CORS preflight returns **403**, so every nautical tile failed while buoys/AIS still painted. Web tiles now use the browser's own headers; OSM is the fallback if Traficom 400s. Native Drift cache unchanged.
